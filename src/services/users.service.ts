...
import { User, UserFlags } from "@/interfaces/users.interface";
...
class UserService {
  ...
  public async changePaswword(req: RequestWithUser): Promise<any> {
    ...
    if (!isPasswordMatching)
      throw new HttpException(409, "기존 비밀번호가 올바르지 않습니다");
  ...
  public async updateRole(req: RequestWithUser): Promise<any> {
    ...
    if (userId === req.user._id.toString())
      throw new HttpException(404, "자신은 수정이 불가능합니다");
    ...
    const getFlag = () => {
      if (role === "teacher") {
        ...
        return UserFlags.busdriver;
      } else if (role === "admin") {
        return UserFlags.teacher + UserFlags.admin + UserFlags.busdriver;
      }
    };
